...
def register_token():
    ...
    if token:
        ...
        try:
            supabase.table("push_tokens").insert({"token": token}).execute()
        except:
            pass  # Token ya existe
...
def status():
    ...
    # Último sensor de gas
    gas_result = supabase.table("sensor_events").select("*").eq("type", "gas").order("timestamp", desc=True).limit(1).execute()
    
    # Último sensor de temperatura
    temp_result = supabase.table("sensor_events").select("*").eq("type", "temperature").order("timestamp", desc=True).limit(1).execute()
    
    # Pánico en últimos 5 minutos
    five_min_ago = (datetime.utcnow() - timedelta(minutes=5)).isoformat()
    panic_result = supabase.table("panic_events").select("*", count="exact").gte("timestamp", five_min_ago).execute()
    
    gas_row = gas_result.data[0] if gas_result.data else None
    temp_row = temp_result.data[0] if temp_result.data else None
    panic_count = len(panic_result.data) if panic_result.data else 0
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>KitchenGuard API v2.0 · Supabase PostgreSQL · <a href="https://kitchenguard-six.vercel.app">Web App</a> · <a href="https://github.com/AleRodriguezCruz/KitchenGuard_API">GitHub</a></p>
        </div>
    </div>
